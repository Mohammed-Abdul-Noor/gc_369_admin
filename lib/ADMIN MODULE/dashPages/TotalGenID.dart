...
class TotalGenID extends StatefulWidget {
  ...
  @override
  State<TotalGenID> createState() => _TotalGenIDState();
}

class _TotalGenIDState extends State<TotalGenID> {
  ...
  next() {
    ...
    if (lastDoc == null || pageIndex == 0) {
      ...
      pageIndex=0;
      userStream =
          FirebaseFirestore.instance.collection('Users').where('motherId',isEqualTo:false)
              .orderBy('index')
              .limit(10).snapshots();
    } else {
      ind+=10;
      userStream = FirebaseFirestore.instance
          .collection('Users').where('motherId',isEqualTo:false)
          .orderBy('index')
