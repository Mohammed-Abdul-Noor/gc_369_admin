                                    ...
                                    rows: List.generate(data.length, (index) {
                                      ...
                                      return DataRow(cells: [
                                        ...
                                        DataCell(SelectableText(
                                            charityProof['senderId'])),
                                        DataCell(Text(
                                            "${DateFormat('dd-MMM-yyyy').format(charityProof['sendTime'].toDate())}")),
                                        DataCell(CachedNetworkImage(
                                          imageUrl: charityProof['file'],
...
getHelp(List<DocumentSnapshot> data, int index, BuildContext context,
  ...
  if (planMap.keys.length < 2) {
    ...
    print(planMap[sendUsermodel.sno.toString()]
        [sendUsermodel.currentPlanLevel.toString()]);
  }
  transaction = planMap[sendUsermodel.sno.toString()]
      [sendUsermodel.currentPlanLevel.toString()];
  if (transaction['amt'] ==
...
getCharity(Map<String, dynamic> transaction, List<DocumentSnapshot> data,
    int index, UserModel sndUsr) {
  if (transaction['cnt'] == sndUsr.currentCount! + 1 &&
      planMap[sndUsr.sno.toString()]['last'] == currentuser?.currentPlanLevel) {
